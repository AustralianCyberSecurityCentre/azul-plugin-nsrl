...
RUN pip install \
    ...
    azul-plugin-nsrl==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-plugin-nsrl==$(cd /tmp/src && python -m setuptools_scm) && pip install nsrl-lookup-server --find-links /tmp/ --pre --upgrade --no-deps;fi
